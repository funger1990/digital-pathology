...
class PathologyDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            x1, multi_x2 = self.training_transform(x1, x2)
        elif self.mode == 'inference':
            x1, multi_x2 = self.inference_transform(x1, x2)

        y = self.df['msi'].iloc[idx]
        info = dict(self.df.iloc[idx, :])

        return x1, multi_x2, y, info

    def training_transform(self, x1, x2):
        # 2.5X
        ...
        multi_x2 = []
        for i in range(self.num_small_patches):
            sub_x2 = transform(x2)
            multi_x2.append(sub_x2)
        multi_x2 = torch.stack(multi_x2)

        return x1, multi_x2

    def inference_transform(self, x1, x2):
        # flip and rotate
        if self.flip:
        ...
        # 2.5X
        x1 = torchvision.transforms.functional.center_crop(x1, 224)
        x1 = torchvision.transforms.functional.to_tensor(x1)

        # 10X
        multi_x2 = []
        for top, left in zip([0, 0, 800, 800, 288, 288, 512, 512], [0, 800, 0, 800, 288, 512, 288, 512]):
            sub_x2 = torchvision.transforms.functional.crop(x2, top, left, 224, 224)
            sub_x2 = torchvision.transforms.functional.to_tensor(sub_x2)
            multi_x2.append(sub_x2)
        multi_x2 = torch.stack(multi_x2)
...
class MultiscaleNet(torch.nn.Module):
    ...
    def forward(self, x1, multi_x2):
        fmap1 = self.cnn1(x1)

        flat_x2 = multi_x2.reshape(-1, 3, 224, 224)
        flat_fmap2 = self.cnn2(flat_x2)
        multi_fmap2 = flat_fmap2.reshape(-1, self.num_small_patches, 512)
        fmap2 = multi_fmap2.mean(dim=1)
